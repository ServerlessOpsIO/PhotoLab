...
```bash
sam local invoke MyFunctionName --event events/MyFunctionName-other.json
```
...
```bash
sam logs -n MyFunctionName --stack-name PhotoLab --tail
```
...
```bash
aws cloudformation delete-stack --stack-name PhotoLab
```
